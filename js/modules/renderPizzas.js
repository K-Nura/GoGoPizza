...
const createCard = (data) => {
  ...
  <div class="card__content">
    ...
    <button class="card__button" type="button" data-id="${data.id}">Выбрать</button>
  </div>
  `;
...
export const renderPizzas = async (toppings) => {
  const pizzas = await getData(`https://gogopizza-products.onrender.com/api/products${toppings ? `?toppings=${toppings}` : ''}`);
  ...
  if (pizzas.length) {
    ...
    modalController({
      ...
      async cbOpen(btnOpen) {
        const pizza = await getData(`https://gogopizza-products.onrender.com/api/products/${btnOpen.dataset.id}`,);
