...
const renderCartList = async () => {
  ...
  if (cartControl.cartData.length) {
    ...
    const cardsData = await Promise.all(cartControl.cartData.map(
      async item => await getData(`https://gogopizza-products.onrender.com/api/products/${item.id}`),
    ...
    cartListElem.textContent = `Корзина пуста`;
...
const submitOrder = async (e) => {
  ...
  try {
    const response = await fetch('https://gogopizza-products.onrender.com/api/orders/', {
      method: 'POST',
      headers: {
    ...
    cartForm.innerHTML = `<h3>Заказ оформлен, номер заказа ${order.orderId}</h3>`;
